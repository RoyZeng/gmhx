...
<script type="text/javascript">
	var dataGrid;
	$(function(){
		onStart(document.body.clientHeight - 260);
		 ...
		 dataGrid = $('#dataGrid').datagrid({
	            ...
	            columns : [ [ 
	              {field : 'apply_id', title:'申请单号',width : 30, align:'center'
	            	...
	            	formatter:function(value, row, index){ 
	              	      return checkVal(row.center_check_result);
	                }
	            },{field : 'headquarters_check_result', title:'总部是否<br/>通过审批',width : 15, align:'center',
	            	formatter:function(value, row, index){ 
	              	      return checkVal(row.headquarters_check_result);
	                }
	...
	function deleteTicket(applyId){
		$.messager.confirm("", "您确定要删除吗？", function (data) {
    		if(data){
    			$.post("${ctx}/retreatReplacementCreate/deleteServiceTicket/"+ applyId, 
        				function(msg){
...
<body>
    <div class="easyui-layout" data-options="fit:true,border:false">
        <div data-options="region:'north', title:'查询条件',border:false,collapsible:true,onBeforeCollapse:function(){clos();},onBeforeOpen:function(){ope();}"  style="height: 260px; overflow: hidden;">
		<form id="searchForm">
			<table style="width:100%; padding: 10px 10px 10px 10px;">
				<tr>
					<td width="8%">申请单号</td>
					<td width="12%"><input style="width:85%" name="applyId" type="text" placeholder="请输入申请单号" class="easyui-validatebox span2" value=""/></td>
					<td width="8%">申请单位</td>
					<td width="12%"><input style="width:85%" name="applyUnit" type="text" placeholder="请输入申请单位" class="easyui-validatebox span2" value=""/></td>
					<td width="8%">销售分部</td>
					<td width="12%"><input style="width:85%" name="sellSubsection" type="text" placeholder="请输入销售分部" class="easyui-validatebox span2" value=""/></td>
					<td width="8%">销售门店</td>
					<td width="12%"><input style="width:85%"  name="sellStore" type="text" placeholder="请输入销售门店" class="easyui-validatebox span2" value=""/></td>
					<td width="8%">是否售前机</td>
					<td width="12%"><input class="easyui-combobox" name="isPresale" data-options="url:'${ctx}/hxCode/getCombobox/sf',panelHeight:'auto',editable:false "/></td>
				</tr>
				<tr>
					<td>是否上墙</td>
					...
					<td>用户名称</td>
					<td><input style="width:85%"  name="customerName" type="text" placeholder="请输入用户名称" class="easyui-validatebox span2" value=""/></td>
					<td>用户电话</td>
					<td><input style="width:85%"  name="customerPhone" type="text" placeholder="请输入用户电话" class="easyui-validatebox span2" value=""/></td>
					<td>退换机</td>
				...
				<tr>
					<td>故障现象</td>
					<td><input style="width:85%"  id="faultPhenomenon" type="text" placeholder="请输入故障现象" class="easyui-validatebox span2" value=""/></td>
					<td>故障原因</td>
					<td><input style="width:85%"  name="faultReason" type="text" placeholder="请输入故障总原因" class="easyui-validatebox span2" value=""/></td>
					<td>故障原因详细描述</td>
					<td><input style="width:85%"  name="faultReasonDetail" type="text" placeholder="请输入故障原因详细描述" class="easyui-validatebox span2" value=""/></td>
					<td>原提货单号</td>
					<td><input style="width:85%" name="deliveryOrderNum" type="text" placeholder="请输入原提货单号" class="easyui-validatebox span2" value=""/></td>
					<td>原机型</td>
					<td><input style="width:85%" name="oldMachineType" type="text" placeholder="请输入原机型" class="easyui-validatebox span2" value=""/></td>
				</tr>
				<tr>
					<td>国美代码(原)</td>
					...
					<td>原机器条码</td>
					<td><input style="width:85%" name="oldMachineCode" type="text" placeholder="请输入原机器条码" class="easyui-validatebox span2" value=""/></td>
					<td>原内机条码1</td>
					<td><input style="width:85%" name="oldInnerCode1" type="text" placeholder="请输入原内机条码1" class="easyui-validatebox span2" value=""/></td>
					<td>原内机条码2</td>
					<td><input style="width:85%" name="oldInnerCode2" type="text" placeholder="请输入原内机条码2" class="easyui-validatebox span2" value=""/></td>
				</tr>
				<tr>
					<td>原外机条码</td>
					<td><input style="width:85%" name="oldOuterCode" type="text" placeholder="请输入原外机条码" class="easyui-validatebox span2" value=""/></td>
					<td>现机型</td>
					<td><input style="width:85%" name="machineType" type="text" placeholder="请输入现机型" class="easyui-validatebox span2" value=""/></td>
					<td>国美代码(新)</td>
					...
					<td>现机器条码</td>
					<td><input style="width:85%" name="machineCode" type="text" placeholder="请输入现机器条码" class="easyui-validatebox span2" value=""/></td>
				</tr>
				<tr>
					<td>现内机条码1</td>
					<td><input style="width:85%"  name="innerCode1" type="text" placeholder="请输入现内机条码1" class="easyui-validatebox span2" value=""/></td>
					<td>现内机条码2</td>
					<td><input style="width:85%"  name="innerCode2" type="text" placeholder="请输入现内机条码2" class="easyui-validatebox span2" value=""/></td>
					<td>现外机条码</td>
					<td><input style="width:85%" name="outerCode" type="text" placeholder="请输入现外机条码" class="easyui-validatebox span2" value=""/></td>
					<td>状态</td>
				...
				<tr>
					<td>机审错误原因</td>
					<td><input style="width:85%" name="machineCheckFaultReason" type="text" placeholder="请输入机审错误原因" class="easyui-validatebox span2" value=""/></td>
					<td>分部是否通过审批</td>
					...
					<td>接收分部</td>
					<td><input style="width:85%" id="receiveCenter" type="text" placeholder="请输入接收分部" class="easyui-validatebox span2" value=""/></td>
					<td width="8%">购机日期</td>
					<td width="12%">
						<input style="width:38%" size="12" id="buyDateStartTime" placeholder="起始日期" class="Wdate" name="buyDateStartTime" type="text" readonly="readonly"  onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'buyDateEndTime\')}'})"/>-
						<input style="width:38%" size="12" id="buyDateEndTime" placeholder="结束日期" class="Wdate" name="buyDateEndTime" type="text" readonly="readonly"  onFocus="WdatePicker({minDate:'#F{$dp.$D(\'buyDateStartTime\')}'})"/> 
					</td>
				</tr>
				<tr>
					<td width="8%">申请日期</td>
					<td width="12%">
						<input style="width:38%" size="12" id="applyDateStartTime" placeholder="起始日期" class="Wdate" name="applyDateStartTime" type="text" readonly="readonly"  onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'applyDateEndTime\')}'})"/>-
						<input style="width:38%" size="12" id="applyDateEndTime" placeholder="结束日期" class="Wdate" name="applyDateEndTime" type="text" readonly="readonly"  onFocus="WdatePicker({minDate:'#F{$dp.$D(\'applyDateStartTime\')}'})"/> 
					</td>
					<td width="8%">安装日期</td>
					<td width="12%">
						<input style="width:38%" size="12" id="installDateStartTime" placeholder="起始日期" class="Wdate" name="installDateStartTime" type="text" readonly="readonly"  onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'installDateEndTime\')}'})"/>-
						<input style="width:38%" size="12" id="installDateEndTime" placeholder="结束日期" class="Wdate" name="installDateEndTime" type="text" readonly="readonly"  onFocus="WdatePicker({minDate:'#F{$dp.$D(\'installDateStartTime\')}'})"/> 
					</td>
