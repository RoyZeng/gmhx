...
<script type="text/javascript">
	...
	$(function(){
		...
		if(category.indexOf("R03")>-1||category.indexOf("r03")>-1){
			$(".kongtiao").show();
			$(".reshuiqi").hide();
			$("#machineCode").val("");
		...
		$('#troubleGrid').datagrid({
			...
			onLoadSuccess : function(data){
				...
				for(var i = 0; i < indexTrouble; i++){
					...
					code = $("tr[id=datagrid-row-r1-2-" + i + "] td[field=troubleCode] input").val();
		...
		$('#partsGrid').datagrid({
			...
			onClickCell : function(rowIndex, field, value){
				...
				parent.$.modalDialog({
					...
					height : 250,
					closable : false,
					href : "${ctx}/repairReceipt/selectFaultView?machineType="+machineType+"&faultCode="+faultCode,
					buttons : [ {
						text : '确定',
						handler : function() {
...
<form id="formCustomer" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		...
		<tr>
			...
			<td><input name="province" type="text" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getRegionCombobox',panelHeight:'200', editable:true,required:true, hasDownArrow:false,mode:'remote'" value="${map.province }"></td>
			<td>区号</td>
...
<form id="formProduct" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		<tr>
			...
			<td  width="30%">
				<input class="easyui-combobox" value="${map.category}" id="category" name="category" data-options="url:'${ctx}/hxCode/getCategoryCombobox',onSelect:function(rec){selectCategory(rec)},panelHeight:'100', editable:false,required:true"/>
			</td>
			<td width="20%">模式</td>
			<td width="30%"><input id="mode" value="${map.mode}" name="mode" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/thms',onSelect:function(rec){selectMode(rec)},panelHeight:'auto'"/></td>
		</tr>
		
		<tr>
			<td width="20%">机型<font color="red">*</font></td>
			<td width="30%"><input id="machineType" name="machineType" class="easyui-combobox" value="${map.machine_type}" data-options="required:true,valueField:'value',textField:'text',url:'${ctx}/hxCode/getModelCombobox?type=-Z',onSelect: function(rec){fillForm(rec)},panelHeight:'auto'"/></td>
			<td width="20%">国美代码</td>
			<td width="30%"><input id="gomeCode" class="easyui-combobox" name="gomeCode" disabled="disabled" value="${map.gome_code}" data-options="url:'${ctx}/hxCode/getCombobox/gmdm',panelHeight:'auto', editable:false" /></td>
		</tr>
		<tr>
			<td>品牌</td>
			<td><input id="brand" id="brand" name="brand" class="easyui-combobox" value="${map.brand}" disabled="disabled" data-options="url:'${ctx}/hxCode/getCombobox/pp',panelHeight:'auto', editable:false"/></td>
			<td class="reshuiqi">条码<font color="red">*</font></td>
			<td class="reshuiqi"><input id="machineCode" name="machineCode" value="${map.machine_code }" type="text" data-options="required:true" value="" class="easyui-validatebox span2"></td>
			<td class="kongtiao">内机条1<font color="red">*</font></td>
			<td class="kongtiao"><input id="internalMachineCode1" value="${map.internal_machine_code1 }" name="internalMachineCode1" type="text" data-options="required:true" value="" class="easyui-validatebox span2"></td>
		</tr>
		<tr class="kongtiao">
			<td>内机条码2</td>
			<td><input id="internalMachineCode2" name="internalMachineCode2" value="${map.internal_machine_code2 }" type="text" value=""  class="easyui-validatebox span2"></td>
			<td>外机条码<font color="red">*</font></td>
			<td><input id="externalMachineCode" name="externalMachineCode" value="${map.external_machine_code }" type="text" value="" data-options="required:true" class="easyui-validatebox span2"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input name="installUnit" type="text" disabled="disabled" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getWebsiteCombobox',required:true" value="${map.install_unit}"></td>
		</tr>
		<tr>
			<td>销售分部<font color="red">*</font></td>
			<td><input name="saleCenter" type="text"  class="easyui-combobox" data-options="url:'${ctx}/hxCode/getECCombobox',required:true,onSelect:function(rec){fillCombo(rec)}" value="${map.sale_center }"></td>
			<td>销售门店<font color="red">*</font></td>
			<td><input id="saleMarket" name="saleMarket" type="text" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getStoreCombobox',panelHeight:'auto', editable:true,required:true" value="${map.sale_market }"></td>
		</tr>
		<tr>
			<td>销售价格</td>
			<td><input name="salePrice" type="text" value="${map.sale_price }"  class="easyui-validatebox span2"></td>
			<td>安装省份<font color="red">*</font> </td>
			<td><input name="installProvince" type="text" value="${map.install_province }" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getRegionCombobox',panelHeight:'200', editable:true,required:true, hasDownArrow:false,mode:'remote'"></td>
		</tr>
		<tr>
			<td>安装详细地址<font color="red">*</font></td>
			<td colspan="3"><textarea name="installDetailAddress" class="easyui-validatebox span2" data-options="required:true" rows="2" cols="80">${map.install_detail_address}</textarea></td>
		</tr>
		<tr>
			<td>备注</td>
...
<form id="formTicket" method="post">
	...
	<table border=1 style="cellSpacing:1;cellPadding:3;width:100%;background-color:#eff5ff;border-collapse:collapse">
		...
		<tr>
			...
			<td>
				<input name="isReplaceParts" class="easyui-combobox" data-options="url:'${ctx}/hxCode/getCombobox/sf?value=${map.is_replace_parts}', panelHeight:'auto',editable:false,required:true"/>
			</td>
		</tr>
		<tr>
			<td>手工单号</td>
