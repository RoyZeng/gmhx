...
<script type="text/javascript">
                    ...
                    columns : [ [
                            {
								...
								width : 5,
								formatter : function(value, row, index) {
										return '<a href="#" onclick="statusOperator(\'' + row.listNumber + '\',\'' + row.processInstanceId  + '\');">【查看】</a>';
								}
                            }] ],
							loadMsg : "数据加载中...",
	...
	function statusOperator(listNumber, processInstanceId) {
		window.location.href = "${ctx}/hxWorkFlow/statusOperator?listNumber="
				+ listNumber + "&processInstanceId=" + processInstanceId+"&type=2";
	}
	
	function exportBatchMP08Excel() {
		if(($("#ksrq").val()==null || $("#ksrq").val()=="") && ($("#jsrq").val()==null || $("#jsrq").val()==""))
			$.messager.alert("警告", "请选择申请开始时间或申请结束时间！");
		else
		    window.location.href = "${ctx}/hxWorkFlow/exportBatchMP08Excel?"+ encodeURI($('#searchForm').serialize());
	}
...
<body class="easyui-layout" style="padding: 10px">
	<div class="easyui-layout" data-options="fit:true,border:false">
		...
		<a href="javascript:void(0);" class="easyui-linkbutton"
			data-options="iconCls:'icon-excel',plain:true" onclick="exportBatchMP08Excel();">网点物料保外销售导出</a>
	</div>
