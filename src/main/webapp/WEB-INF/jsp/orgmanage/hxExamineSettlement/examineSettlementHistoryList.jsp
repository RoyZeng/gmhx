...
<script type="text/javascript">
	...
	function onStart(heigh){
	 	dataGrid = $('#dataGrid').datagrid({
			title : "安装服务单",
			url : "${ctx}/hxExamineSettlement/getExamineSettlementHistoryPageList",
			striped : true,
            height : heigh,
            collapsible : true,
            autoRowHeight : false,
			...
			columns : [ [ 
			   {field : 'service_unit',title : '服务单位', width : 10 ,align:'center'
				...
				formatter : function(value, row, index) {
					return '<a href="#" onclick="checkTicket(\'' + row.settlement_id + '\');">查看</a>';
				}
			} ] ],
			toolbar : '#toolbar',
	...
	function checkTicket(settlementId){
		window.location.href="${ctx}/hxExamineSettlement/examineSettlementHistoryShow/"+settlementId;
	}
</script>
</head>
<body>
