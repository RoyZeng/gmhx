...
<script type="text/javascript">
	$(function() {
		...
		$('#importForm').form({
			url : '${ctx}/hxPositionMapping/importExcel',
			onSubmit : function() {
			...
			success : function(msg) {
				parent.$.messager.progress('close');
				$.messager.alert('提示:','失败'+$.parseJSON(msg).failure+'行,原因是所有字段值都不能为空，或者是服务器异常导致失败!');
				$.messager.alert('提示:','已存在岗位编码:'+$.parseJSON(msg).exists);
	...
	function goBack() {
		window.location.href = "${ctx}/hxPositionMapping/hxBarCodeView";
	}
	
	function downloadTemplate(){
		window.location.href = "${ctx}/hxPositionMapping/downloadTemplate";
	}


</script>
<div class="easyui-panel" title="岗位导入" style="width: auto">
	<div id="toolbar" style="display: run-in;">
		...
		<form id="importForm" method="post" enctype="multipart/form-data" action="${ctx}/hxPositionMapping/importExcel">
