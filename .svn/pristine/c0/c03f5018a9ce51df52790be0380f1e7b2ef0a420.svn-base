...
<script type="text/javascript">
	...
	function onStart(heigh){
	 	dataGrid = $('#dataGrid').datagrid({
			title : "安装服务单",
			url : "${ctx}/hxExamineSettlement/getComfirmSettlementPageList",
			striped : true,
            height : heigh,
            collapsible : true,
            autoRowHeight : false,
			...
			columns : [ [ 
			   {field : 'service_unit',title : '服务单位', width : 10 ,align:'center'
				...
				formatter : function(value, row, index) {
					return '<a href="#" onclick="comfirm(\'' + row.settlement_id + '\');">确认</a>';
				}
			} ] ],
			toolbar : '#toolbar',
	...
	function comfirm(settlementId){
		window.location.href="${ctx}/hxExamineSettlement/confirmSettlementView/"+settlementId;
	}
	
	function update(settlementId){
		window.location.href="${ctx}/hxExamineSettlement/confirmSettlementUpdateView/"+settlementId;
	}
	
	function checkOut(settlementId){
		window.location.href="${ctx}/hxExamineSettlement/examineSettlementDetail/"+settlementId;
	}
