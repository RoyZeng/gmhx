...
<script type="text/javascript">
	...
	function query(){
		if (($("#queryName").val() == null || $("#queryName").val() == "")
			...
			parent.$.modalDialog({
				...
				height : 400,
				closable : false,
				href : '${ctx}/repairReceipt/selectCustomer?'+encodeURI($('#query').serialize()),
				buttons : [{
					text : '确定',
					handler : function() {
						var customerId = parent.$.modalDialog.handler.find('#checkedId').val();
						parent.$.modalDialog.handler.dialog('close');
						window.location.href="${ctx}/repairReceipt/getCustomer?customerId="+ customerId; 
					}
				},{
					text : '取消',
					handler : function() {
	...
	function newCustomer(){
		window.location.href = "${ctx}/repairReceipt/addRepairReceiptView";
	}
		...
		<form id="query" method="post">
			<table class="table table-hover table-condensed" width="100%;" style="padding:20px 0 10px 60px">
				<tr>
					...
					<td width="20%">联系电话</td>
					<td width="30%"><input id="queryphone" name="queryphone" type="text" placeholder="请输入联系电话" class="easyui-validatebox span2" value=""></td>
				</tr>
				<tr>
					<td>机器条码</td>
